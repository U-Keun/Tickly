<script lang="ts">
  ...
  import TagSuggestionBoard from './TagSuggestionBoard.svelte';
  ...
  let activeTagToken = $state<InlineTagToken | null>(null);
  ...
  let activeTagName = $derived(normalizeTagName(activeTagToken?.query ?? ''));
  let tagSuggestions = $derived(
    activeTagToken
      ? suggestTags(availableTags, activeTagToken.query, 6).filter(
  ...
  let submitTagNames = $derived(normalizeTagNames(selectedTagNames));
  ...
  function updateActiveTagToken(): void {
    ...
    activeTagToken = getActiveTagToken(addInput, inputElement.selectionStart);
  }
  ...
  function handleInput(event: Event): void {
    ...
    if (mode === 'search') {
      ...
      addInput = value;
      activeTagToken = getActiveTagToken(
        value,
  ...
  async function handleSubmit(): Promise<void> {
    ...
    try {
      ...
      // The checklist store owns the visible error banner; keep the draft in place.
    } finally {
      isSubmitting = false;
      focusInput();
    }
  ...
  async function prepareTagInput(): Promise<void> {
    ...
    const currentToken = getActiveTagToken(addInput, caretIndex);
  ...
  async function selectTagSuggestion(tag: Tag): Promise<void> {
  ...
  async function addSelectedTag(tagName: string): Promise<void> {
    ...
    const replacement = removeTagToken(addInput, activeTagToken);
    addInput = replacement.value;
    selectedTagNames = normalizeTagNames([...selectedTagNames, tagName]);
    activeTagToken = null;
    await tick();
    inputElement?.focus({ preventScroll: true });
  ...
  >
    {#if mode === 'search'}
      ...
      >
        <button
          ...
          aria-label={i18n.t('checklistExitSearch')}
          title={i18n.t('checklistExitSearch')}
    ...
    {:else}
      <button
        ...
        aria-label={i18n.t('checklistAddItem')}
        title={i18n.t('checklistAddItem')}
    ...
    <input
      ...
      placeholder={mode === 'search' ? i18n.t('checklistSearchPlaceholder') : i18n.t('checklistNewItemPlaceholder')}
      aria-label={mode === 'search' ? i18n.t('checklistSearchPlaceholder') : i18n.t('checklistNewItemPlaceholder')}
      autocomplete="off"
      {disabled}
      oninput={handleInput}
      onfocus={handleFocus}
    ...
    {#if mode === 'search' && trimmedInput}
      <button
        ...
        aria-label={i18n.t('checklistClearInput')}
        title={i18n.t('checklistClearInput')}
        onclick={clearInput}
      >
    ...
    {#if mode === 'add'}
      <div class="flex flex-shrink-0 items-center gap-1.5">
        <button
          ...
          aria-label={i18n.t('checklistAddTag')}
          title={i18n.t('checklistAddTag')}
          disabled={disabled || isSubmitting}
        ...
        <button
          type="button"
          class="grid h-10 w-10 place-items-center rounded-[13px] text-[var(--color-ink)] transition-colors hover:bg-[var(--color-canvas)] active:bg-[var(--color-mist)] disabled:cursor-not-allowed disabled:opacity-40"
          aria-label={i18n.t('checklistEnterSearch')}
          title={i18n.t('checklistEnterSearch')}
          disabled={disabled}
