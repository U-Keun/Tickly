<script lang="ts">
  ...
  let title = $derived(
    mode === 'create' ? i18n.t('checklistCreateCategoryTitle') : i18n.t('checklistRenameCategoryTitle')
  );
...
>
  <form class="flex flex-col gap-4" onsubmit={submitCategory}>
    <label class="flex flex-col gap-2">
      <span class="text-sm font-semibold text-[var(--color-ink)]">{i18n.t('checklistCategoryNameLabel')}</span>
      <input
        use:iosFocusFix
        ...
        aria-label={i18n.t('checklistCategoryNameLabel')}
        autocomplete="off"
        disabled={isSaving}
      />
    </label>
