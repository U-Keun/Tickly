<script lang="ts">
  ...
  interface Props {
    ...
    isLoading?: boolean;
    onSelectResult: (result: ItemSearchResult) => MaybePromise;
  }
  ...
  function getMemoSearchSnippet(result: ItemSearchResult): string | null {
  ...
  function getTagSearchSnippet(result: ItemSearchResult): string | null {
...
<div
  ...
  role="region"
  aria-label={i18n.t('checklistSearchSuggestions')}
>
  <div class="flex items-center gap-2 px-2 pb-1 pt-1 text-xs font-semibold text-[var(--color-ink-muted)]">
    ...
    <span>{i18n.t('checklistSearchSuggestions')}</span>
  </div>

  {#if isLoading}
    <div class="flex min-h-11 items-center rounded-[12px] px-3 text-sm text-[var(--color-ink-muted)]">
      {i18n.t('checklistSearching')}
    </div>
  {:else if results.length === 0}
    <div class="flex min-h-11 items-center rounded-[12px] px-3 text-sm text-[var(--color-ink-muted)]">
      {i18n.t('checklistNoSearchResultsTemplate')(query)}
    </div>
  {:else}
          ...
          <span class="min-w-0 flex-1">
            ...
            {#if memoSnippet}
              <span class="mt-0.5 block truncate text-xs font-medium text-[var(--color-ink-muted)]">
                {i18n.t('checklistMemoSearchSnippetTemplate')(memoSnippet)}
              </span>
            {:else if tagSnippet}
              <span class="mt-0.5 block truncate text-xs font-medium text-[var(--color-ink-muted)]">
                {i18n.t('checklistTagSearchSnippetTemplate')(tagSnippet)}
              </span>
            {/if}
          </span>
