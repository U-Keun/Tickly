<script lang="ts">
  ...
  interface Props {
    ...
    repeatDetail?: number[];
    disabled?: boolean;
    onChange: (repeatType: RepeatType, repeatDetail: number[]) => MaybePromise;
  }
  ...
  const repeatLabelKeys: Record<RepeatType, 'repeatNone' | 'repeatDaily' | 'repeatWeekly' | 'repeatMonthly'> = {
    none: 'repeatNone',
    daily: 'repeatDaily',
    weekly: 'repeatWeekly',
  ...
  let visualRepeatType = $state<RepeatType>('none');
  let visualRepeatDetail = $state<number[]>([]);
  let normalizedDetail = $derived(normalizeRepeatDetail(visualRepeatType, visualRepeatDetail));
  let currentDetailType = $derived(detailTypeFor(visualRepeatType));
  let renderedDetailType = $state<Extract<RepeatType, 'weekly' | 'monthly'> | null>(null);
  let renderedNormalizedDetail = $derived(
    renderedDetailType ? normalizeRepeatDetail(renderedDetailType, repeatDetail) : []
  );
  ...
  $effect(() => {
    if (
      ...
      normalizeRepeatDetail(visualRepeatType, visualRepeatDetail).join(',') !==
        normalizeRepeatDetail(repeatType, repeatDetail).join(',')
    ) {
      visualRepeatType = repeatType;
  ...
  function detailTypeFor(type: RepeatType): Extract<RepeatType, 'weekly' | 'monthly'> | null {
  ...
  function selectRepeatType(nextType: RepeatType): void {
    if (disabled) return;
    ...
    const normalizedNextDetail = normalizeRepeatDetail(nextType, nextDetail);
  ...
  function toggleDetail(value: number): void {
    ...
    const normalizedNextDetail = normalizeRepeatDetail(visualRepeatType, nextDetail);
    visualRepeatDetail = normalizedNextDetail;
...
<div class="flex flex-col gap-[10px] rounded-[14px] border-2 border-[var(--color-ink)] bg-[var(--color-paper)] p-3">
  <div class="flex items-center gap-[7px] text-sm font-semibold text-[var(--color-ink)]">
    ...
    <span>{i18n.t('checklistItemRepeatLabel')}</span>
  </div>

  <div class="grid grid-cols-4 gap-2">
    {#each REPEAT_TYPES as type (type)}
      <button
        type="button"
        ...
        <div use:measureDetailContent class="repeat-detail-content flex flex-col gap-[7px]">
          {#if renderedDetailType === 'weekly'}
            ...
            <div class="grid grid-cols-7 gap-1.5">
              {#each WEEKDAY_VALUES as day (day)}
                <button
                  type="button"
          ...
          {:else}
            ...
            <div class="grid grid-cols-7 gap-1.5">
              {#each MONTH_DAY_VALUES as day (day)}
                <button
                  type="button"
