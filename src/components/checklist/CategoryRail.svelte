<script lang="ts">
  ...
  interface Props {
    ...
    onReorderConsider: (categories: Category[]) => void;
    onReorderFinalize: (categories: Category[]) => MaybePromise;
  }
  ...
  async function selectCategory(id: number): Promise<void> {
    ...
    try {
      ...
      // The checklist store owns the visible error banner; keep the rail state stable.
    }
  }
  ...
  function handleReorderConsider(event: CustomEvent<DndEvent<Category>>): void {
    onReorderConsider(event.detail.items);
  }

  async function handleReorderFinalize(event: CustomEvent<DndEvent<Category>>): Promise<void> {
    try {
      await onReorderFinalize(event.detail.items);
    } catch {
      // The checklist store owns the visible error banner; keep the rail interactive.
    }
  }
    ...
    <div class="scrollbar-hide min-w-0 overflow-x-auto">
      <div class="relative min-w-full">
        ...
        <div
          ...
          aria-label={i18n.t('checklistCategories')}
        >
          {#each categories as category (category.id)}
            ...
            >
              <button
                ...
                title={isReorderMode ? i18n.t('checklistCategoryOrderHint') : category.name}
  ...
  <div class="flex shrink-0 items-center gap-1.5">
    {#if isReorderMode}
      <button
        ...
        aria-label={i18n.t('checklistFinishCategoryOrder')}
        title={i18n.t('checklistFinishCategoryOrder')}
        disabled={isReorderBusy}
        onclick={onFinishReorderMode}
      >
        <Check size={17} strokeWidth={2.6} aria-hidden="true" />
        <span>{i18n.t('checklistFinishCategoryOrder')}</span>
      </button>
    {:else}
      <button
        type="button"
        ...
        aria-label={i18n.t('checklistManageCategory')}
...
<style>
  ...
  .category-wiggle {
    animation: category-wiggle 720ms ease-in-out infinite;
