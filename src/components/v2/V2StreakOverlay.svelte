<script lang="ts">
  ...
  import V2StreakCard from './V2StreakCard.svelte';

  type MaybePromise = void | Promise<void>;

  interface Props {
    show: boolean;
    heatmaps?: V2StreakHeatmap[];
    isLoading?: boolean;
  ...
  >
    <header class="flex shrink-0 items-center gap-3 px-5 pb-3 pt-[max(1rem,var(--safe-area-top))]">
      ...
      <div class="min-w-0 flex-1">
        <h2 class="text-xl font-bold leading-tight">{i18n.t('v2StreakOverlayTitle')}</h2>
        <p class="mt-0.5 text-sm font-semibold text-[var(--color-ink-muted)]">
          {i18n.t('v2StreakOverlaySubtitle')}
        </p>
      </div>
      <button
        type="button"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-5 pb-[max(1.25rem,var(--safe-area-bottom))]">
      ...
      {:else if errorMessage}
        ...
        >
          <p class="text-base font-bold">{i18n.t('v2StreakLoadErrorTitle')}</p>
      ...
      {:else if heatmaps.length === 0}
        ...
        >
          ...
          <p class="mt-3 text-lg font-bold">{i18n.t('v2StreakOverlayEmptyTitle')}</p>
          <p class="mt-1 text-sm font-semibold leading-6 text-[var(--color-ink-muted)]">
            {i18n.t('v2StreakOverlayEmptyMessage')}
          </p>
        </div>
      {:else}
        <div class="flex flex-col gap-3">
          {#each heatmaps as heatmap (heatmap.item.id)}
            <V2StreakCard {heatmap} />
          {/each}
        </div>
      {/if}
    </div>
