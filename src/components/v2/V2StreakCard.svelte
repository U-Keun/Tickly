<script lang="ts">
  ...
  function formatRepeatSummary(): string {
    ...
    if (heatmap.item.repeat_type === 'none') return i18n.t('v2StreakDailyCadence');

    const detail = parseV2RepeatDetail(heatmap.item.repeat_detail);
    ...
    return i18n.t('v2StreakDailyCadence');
  }
</script>
...
<article class="rounded-[6px_24px_6px_24px] border-2 border-[var(--color-ink)] bg-[var(--color-paper)] p-3 shadow-sm">
  ...
  <div class="mt-3 grid grid-cols-3 gap-2 text-center">
    <div class="rounded-[12px] bg-[var(--color-white)] px-2 py-2">
      <p class="text-[11px] font-semibold text-[var(--color-ink-muted)]">{i18n.t('v2StreakTotalDays')}</p>
      <p class="mt-0.5 text-lg font-bold text-[var(--color-ink)]">{heatmap.total_days}</p>
    </div>
    <div class="rounded-[12px] bg-[var(--color-white)] px-2 py-2">
      <p class="text-[11px] font-semibold text-[var(--color-ink-muted)]">{i18n.t('v2StreakCurrent')}</p>
      <p class="mt-0.5 text-lg font-bold text-[var(--color-ink)]">{heatmap.current_streak}</p>
    </div>
    <div class="rounded-[12px] bg-[var(--color-white)] px-2 py-2">
      <p class="text-[11px] font-semibold text-[var(--color-ink-muted)]">{i18n.t('v2StreakLongest')}</p>
  ...
  <div
    ...
    aria-label={i18n.t('v2StreakHeatmapLabel')}
  >
