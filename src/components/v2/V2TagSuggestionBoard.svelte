<script lang="ts">
  ...
  interface Props {
    ...
    onSelectTag: (tag: V2Tag) => MaybePromise;
...
<div
  ...
  role="region"
  aria-label={i18n.t('v2TagSuggestions')}
>
  ...
  {#if createTagName}
    ...
    >
      ...
      <span class="truncate">{i18n.t('v2CreateTagSuggestionTemplate')(createTagName)}</span>
    </button>
  {/if}
  ...
  {#if suggestions.length === 0 && !createTagName && (query || selectedTagNames.length === 0)}
    <div class="flex min-h-11 items-center rounded-[12px] px-3 text-sm text-[var(--color-ink-muted)]">
      {query ? i18n.t('v2NoTagSuggestionsTemplate')(query) : i18n.t('v2NoTagsYet')}
    </div>
