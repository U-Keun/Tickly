...
const V2_NOTIFICATION_ID_OFFSET = 200_000_000;
const MAX_I32 = 2_147_483_647;

function notificationId(itemId: number): number | null {
  const id = V2_NOTIFICATION_ID_OFFSET + Math.trunc(itemId);
  return id > V2_NOTIFICATION_ID_OFFSET && id <= MAX_I32 ? id : null;
}

function isV2NotificationId(id: number): boolean {
  return id > V2_NOTIFICATION_ID_OFFSET && id <= MAX_I32;
}
...
async function ensurePermission(): Promise<boolean> {
  try {
    ...
    console.error('V2 notification permission error:', error);
    return false;
  }
}
...
export async function cancelReminderForItem(itemId: number): Promise<void> {
  ...
  try {
    ...
    console.error('Failed to cancel v2 reminder:', error);
  }
}

export async function syncReminderForItem(item: V2TodoItem): Promise<void> {
  ...
  try {
    ...
    console.error('Failed to schedule v2 reminder:', error);
  }
}

export async function syncActiveReminderNotifications(items: V2TodoItem[]): Promise<void> {
  ...
  try {
    ...
    const staleIds = pending
      ...
      .filter((id) => isV2NotificationId(id) && !activeIds.has(id));
    await nativeCancel(staleIds);
  } catch (error) {
    console.error('Failed to prune stale v2 reminders:', error);
  }
