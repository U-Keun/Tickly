...
export interface NativeTextSheetOptions {
  title: string;
  label: string;
  placeholder: string;
  initialValue: string;
...
export interface NativeActionSheetAction {
  id: string;
  label: string;
...
export interface NativeRepeatLabels {
  none: string;
  daily: string;
  weekly: string;
  monthly: string;
...
export interface NativeFormSheetField {
  id: string;
  ...
  repeatLabels?: NativeRepeatLabels;
  suggestions?: string[];
  required?: boolean;
...
export interface NativeFormSheetOptions {
  title: string;
...
export type NativeTextSheetResult =
  | { status: 'saved'; value: string }
  | { status: 'cancelled' }
  | { status: 'unavailable' };

export type NativeActionSheetResult =
  | { status: 'action'; actionId: string }
  | { status: 'cancelled' }
  | { status: 'unavailable' };

export type NativeFormSheetResult =
  | { status: 'saved'; values: Record<string, string | string[]> }
  | { status: 'cancelled' }
  | { status: 'unavailable' };

type NativeSheetRequest =
  | {
      token: string;
      kind: 'text';
      title: string;
...
interface NativeSheetEventDetail {
  token: string;
...
function parseEventDetail(detail: unknown): NativeSheetEventDetail | null {
  if (!detail || typeof detail !== 'object') return null;

  const maybeDetail = detail as Partial<NativeSheetEventDetail>;
...
async function openNativeSheet(request: NativeSheetRequest): Promise<NativeSheetResult> {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: NativeSheetResult): void => {
      cleanup();
      resolve(result);
    };
...
export async function openNativeTextSheet(
  options: NativeTextSheetOptions
): Promise<NativeTextSheetResult> {
...
export async function openNativeFormSheet(
  options: NativeFormSheetOptions
...
): Promise<NativeActionSheetResult> {
  const result = await openNativeSheet({
    token: createToken(),
    kind: 'actions',
    title: options.title,
