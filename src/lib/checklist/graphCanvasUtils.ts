...
export type GraphNodeType = 'item' | 'tag';
...
export interface GraphCategoryCell {
  category: Category;
  left: number;
  right: number;
  top: number;
...
export const GRAPH_TAP_THRESHOLD = 7;
...
export function buildGraphSimulationData(
  ...
  categoryCells: GraphCategoryCell[];
  ...
  const itemNodes: GraphSimNode[] = data.items.map((item, index) => {
  ...
  const simLinks = data.tag_edges
    ...
    .filter((link): link is GraphSimLink => link !== null);
