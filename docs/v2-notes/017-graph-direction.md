...
- Items are draggable nodes inside category membranes.
...
- Item nodes are constrained to remain inside their own category membrane; tag hubs may sit between membranes to show cross-category relationships.
...
- Tapping an item node opens the same v2 item detail flow used by checklist rows.
